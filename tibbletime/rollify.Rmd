...
use a list to index by position and name at different levels. Within a list, wrap strings in get-attr() to extract named attributes. If a component is not present, the value of .default will be returned.

* window The window size to roll over
* unlist If the function returns a single value each time it is called, use unlist = TRUE.
If the function returns more than one value, or a more complicated object (like a linear model), use unlist = FALSE to create a list-column of the rolling results.
...
``dplyr::bind_rows()`` is often helpful in these cases to get meaningful output.
...
For each country in the gapminder data, calculate a linear regression every 5 periods of lifeExp ~ year
