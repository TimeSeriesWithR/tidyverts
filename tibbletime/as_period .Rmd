...
<pre><code>
as_period(.tbl_time, period = "yearly", start_date = NULL, side = "start", include_endpoints = FALSE, ...)
</code></pre>

#### Arguments
* ``.tbl_time``: A tbl_time object. period A character specification used for time-based grouping. The general format to
use is "frequency period" where frequency is a number like 1 or 2, and period is an interval like weekly or yearly. There must be a space between the two. Note that you can pass the specification in a flexible way:
